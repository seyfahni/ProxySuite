...
public class Events implements Listener {
    ...
    public void onPostLogin(final PostLoginEvent ev) {
        ...
        main.getProxy().getScheduler().schedule(main, () -> {
            ...
            main.getProxy().getScheduler().schedule(main, () -> {
                ...
                if (main.getPermissionHandler().hasPermission(p, "proxysuite.messages.motd"))
                    main.getConfig().getStringList("ProxySuite.Messages.MOTD")
                            .forEach(s -> main.getMessageHandler().sendMessage(p, s));
            ...
            if (max > 0 && main.getPlayerHandler().getIps().containsKey(p.getName()) && main.getPlayerHandler().getIps().get(ip) >= max) {
                String players = main.getProxy().getPlayers().stream()
                        .filter(pl -> pl != ev.getPlayer())
                        .filter(pl -> pl.getPendingConnection().getAddress().getAddress().toString().equals(ip))
                        .map(player -> player.getName())
                        .collect(Collectors.joining(", "));
                main.getMessageHandler().sendMessageWithPermission(main.getMessageHandler().getMessage("security.join.manyclients.info")
                        .replace("%player%", ev.getPlayer().getName())
                        .replace("%sameIPList%", players), "proxysuite.security.ipjoininfo");
            }
    ...
    public void onChat(ChatEvent ev) {
        if (main.getConfig().getBoolean("ProxySuite.Logging.CommandOutputEnabled") && ev.getSender() instanceof ProxiedPlayer) {
            if (ev.getMessage().startsWith("/")) {
                main.getLogger().log(Level.INFO, "{0} issued command: {1}", new Object[]{ev.getSender(), ev.getMessage()});
            } else {
